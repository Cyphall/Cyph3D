...
public:
     ...
     */
    thread_pool(ui32 _thread_count = std::thread::hardware_concurrency(), GLFWwindow* mainContext = nullptr)
        : thread_count(_thread_count), threads(new std::thread[_thread_count]), _mainContext(mainContext), _contexts(new GLFWwindow*[_thread_count])
    {
        create_threads();
    }
    ...
    void reset(const ui32 &_thread_count = std::thread::hardware_concurrency())
    {
        ...
        _contexts.reset(new GLFWwindow*[thread_count]);
        paused = was_paused;
        create_threads();
        running = true;
    }
...
private:
    ...
    void create_threads()
    {
        for (ui32 i = 0; i < thread_count; i++)
        {
        	...
        	if (_mainContext)
			{
				...
				_contexts[i] = glfwCreateWindow(1, 1, "", NULL, _mainContext);
			}
            threads[i] = std::thread(&thread_pool::worker, this, _contexts[i]);
        }
    }
     ...
     */
	void worker(GLFWwindow* context)
    {
    	if (context)
		{
    ...
    std::unique_ptr<GLFWwindow*[]> _contexts;
	...
	/**
	 * @brief A context used to create a shared context in each thread
	 */
	GLFWwindow* _mainContext;
};
